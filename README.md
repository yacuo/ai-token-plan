# Token Plan、AI API、Coding Plan 价格对比 ｜ AI 订阅与大模型 API 价格目录

面向中国用户整理的 AI Token Plan、Coding Plan 价格、AI 订阅价格、大模型 API 价格、LLM API Pricing、API Key 申请入口、免费额度与模型套餐目录。

如果你正在比较智谱 GLM、通义千问 Qwen、Kimi、豆包 Doubao、MiniMax、DeepSeek、Claude、Gemini、OpenRouter 等 Token Plan 与 API 价格，这个仓库用 GitHub README 做一份精简版对比：看入门价格、套餐类型、主要模型、Token 成本、免费额度、额度重置、API Key 申请和 TokenPlan 详情页。

常见搜索场景包括：Token Plan 价格、Token Plan 对比、AI Token Plan、AI 订阅价格、大模型 API 价格、模型 API 价格对比、LLM API Pricing、API Key 申请、免费额度、额度重置、模型套餐、Claude API 价格、DeepSeek API 价格、Kimi API 价格、Qwen API 价格、OpenRouter API 价格。

本仓库是 [TokenPlan](https://tokenplan.vip/) 的 GitHub 精简版，适合快速浏览、收藏和 Fork。

## 站点来源

官方来源：**TokenPlan**  
### https://tokenplan.vip

可视化站点：**tokenplan.pw**  
### https://tokenplan.pw

完整套餐权益、额度说明和最新价格请查看 TokenPlan。

如果这个仓库帮你少开几个价格页，欢迎 Star，方便下次查看。

## 技术栈

本仓库不是传统后端项目，而是一个面向 GitHub 与静态站点发布的内容型开源项目。

- **Next.js / React**：用于生成可视化站点页面。
- **TypeScript**：用于站点页面、Markdown 解析和内容组织逻辑。
- **Markdown**：厂商页、专题页和 README 均以 Markdown 维护，方便 GitHub 直接阅读和 PR 协作。
- **Tailwind CSS**：用于可视化站点的卡片、文章页、目录和响应式样式。
- **静态导出 / 静态部署**：站点内容可构建为静态页面，适合部署到 Cloudflare Pages、GitHub Pages 或其它 CDN。
- **Shell 脚本**：提供本地安装、构建、预览和部署辅助命令。

## 生成原理

本项目的内容生成流程尽量保持简单、透明，方便开源协作和人工校对。

1. **人工整理数据**：从 TokenPlan 详情页和厂商公开页面整理价格、模型、额度、API 入口、免费额度和适用场景。
2. **Markdown 作为内容源**：每个厂商对应 `vendors/*.md`，专题文章对应 `guides/*.md`，README 保留一份 GitHub 精简版目录。
3. **站点读取 Markdown**：可视化站点在构建时读取 Markdown 文件，解析标题、标签、目录、正文、来源卡片和可视化页面入口。
4. **自动生成页面**：根据 Markdown 文件名生成站内路径，例如 `vendors/qianwen-token-api.md` 会对应可视化页面 `/qianwen-token-api/`。
5. **统一渲染样式**：数据来源、可视化页面、表格、文章目录等组件由站点统一渲染，避免每篇文章重复写 HTML。
6. **静态化输出**：构建后生成可部署的静态页面，方便搜索引擎收录和用户快速访问。

简单理解：**Markdown 负责内容，Next.js 负责把内容变成可视化站点，README 负责 GitHub 首页快速阅读。**

## 快速对比

| 平台 | 类型 | 价格 | 模型 | 关注点 |
|---|---|---|---|---|
| [Claude](vendors/claude-token-plan.md) | 订阅 / API | $20/月；API $1/$5/百万 tokens 起 | Claude | Claude Code、官方生态 |
| [OpenRouter](vendors/openrouter-token-api.md) | API 聚合 | $0.15/$0.60/百万 tokens 参考 | 多模型 | OpenAI 兼容、统一入口 |
| [DeepSeek](vendors/deepseek-token-api.md) | API | ¥1/¥2/百万 tokens 起 | DeepSeek | 低成本、推理 |
| [Kimi](vendors/kimi-token-plan.md) | Token Plan / API | ¥49/月起 | Kimi | 国产套餐、Agent |
| [Qwen](vendors/qianwen-token-api.md) | API | ¥0.3/¥0.6/百万 tokens 起 | Qwen | 阿里云百炼 |
| [MiniMax](vendors/minimax-token-plan.md) | Token Plan / API | ¥49/月起；积分 1000=¥7 | MiniMax | 积分制、多模态 |
| [Gemini](vendors/gemini-token-plan.md) | 订阅 / API | $4.99/月起 | Gemini | Google 生态 |
| [Doubao](vendors/doubao-token-api.md) | API | ¥0.2/¥2/百万 tokens 起 | Doubao | 火山方舟 |
| [GLM](vendors/zhipu-token-plan.md) | Token Plan / API | ¥49/月起 | GLM | 智谱生态 |


## 对比案例

- [LLM API 价格目录](guides/llm-api-pricing.md)：整理 OpenAI API 价格、Claude API 价格、Gemini API 价格、DeepSeek API 价格、Qwen API 价格、Doubao API 价格、GLM API 价格等常见大模型 API 价格方向。
- [AI Token Plan 套餐价格目录](guides/ai-token-plan-pricing.md)：整理 Claude Token Plan、Kimi Token Plan、MiniMax Token Plan、Gemini 订阅、GLM Token Plan 等 AI Token Plan 套餐价格方向。
- [免费额度目录](guides/ai-api-free-tiers.md)：整理大模型 API 免费额度、AI API 免费额度、Token Plan 免费档、开发者控制台试用额度等低成本入口方向。

## 常见查询场景

这个仓库重点覆盖这些搜索需求：AI Token Plan、Token Plan 价格、Token Plan 对比、AI API 价格、LLM API Pricing、大模型 API 价格、模型 API 价格对比、API Key 申请、Claude API 价格、OpenAI API 价格、Gemini API 价格、DeepSeek API 价格、Kimi API 价格、Qwen API 价格、Doubao API 价格、GLM API 价格、MiniMax Token Plan、OpenRouter API 价格。

如果你在做 AI 应用、Agent、知识库、AI 客服、代码生成工具或 SaaS 产品，通常需要同时比较模型价格、Token 成本、API 入口、套餐权益和免费额度。这个 README 只保留适合 GitHub 收藏的精简信息，完整对比请查看 [TokenPlan](https://tokenplan.vip/)。

## 厂商摘要

### Claude Token Plan / Claude API

Claude 价格从 $20/月起，API 参考 $1/$5/百万 tokens 起。适合关注 Claude Opus、Claude Sonnet、Claude Haiku、Claude Code、Anthropic API、Claude API 价格和 Claude Token Plan 的用户。订阅和 API 是不同计费体系，适合分别比较。

### OpenRouter API

OpenRouter 参考价格 $0.15/$0.60/百万 tokens 起，不同模型按 provider 单价变化。适合关注 OpenRouter API 价格、OpenAI 兼容接口、多模型 API 聚合、API Key 统一管理、Claude / OpenAI / Gemini / DeepSeek / Qwen 多模型切换的用户。

### DeepSeek API

DeepSeek API 价格从 ¥1/¥2/百万 tokens 起。适合关注 DeepSeek API 价格、DeepSeek V4-Flash、DeepSeek V4-Pro、国产大模型 API、低成本 Token 调用和推理模型 API 的用户。

### Kimi Token Plan / Kimi API

Kimi 会员价格从 ¥49/月起。适合关注 Kimi Token Plan、Kimi API 价格、Kimi K2.7 Code、Kimi K2.6、Kimi 会员套餐、Agent 和编程套餐的用户。

### Qwen / 通义千问 API

Qwen API 价格从 ¥0.3/¥0.6/百万 tokens 起。适合关注 Qwen API 价格、通义千问 API、阿里云百炼、Qwen Coder、Qwen 多模态模型和国产大模型 API 的用户。

### MiniMax Token Plan / MiniMax API

MiniMax 价格从 ¥49/月起，积分价格 1000=¥7。适合关注 MiniMax Token Plan、MiniMax API 价格、MiniMax-M3、MiniMax-M2.7、积分制套餐、多模态模型和 Agent 调用的用户。

### Gemini API / Gemini 订阅

Gemini 订阅价格从 $4.99/月起，Pro 为 $19.99/月。适合关注 Gemini API 价格、Google AI Pro、Gemini Pro、Gemini Flash、长上下文模型、多模态模型和 Google AI Studio 的用户。

### Doubao / 豆包 API

Doubao API 价格从 ¥0.2/¥2/百万 tokens 起。适合关注 Doubao API 价格、豆包大模型、火山方舟、国产模型 API、Agent 接入和多模态应用的用户。

### GLM / 智谱 API

GLM Token Plan 价格从 ¥49/月起。适合关注 GLM API 价格、智谱大模型、GLM Token Plan、国产模型 API、编程模型和多模态模型的用户。

## 适合谁看

- 正在比较大模型 API 价格的开发者
- 想控制 Token 成本的 AI 应用开发者
- 正在选择 OpenAI / Claude / Gemini / DeepSeek / Kimi / Qwen / Doubao / GLM 的用户
- 想 Fork 一份团队内部模型 API 选型表的人

## 说明

本仓库整理 AI Token Plan、LLM API、模型套餐和 TokenPlan 详情页，作为快速选型和收藏参考。

价格、额度、模型名称和套餐权益可能随时变化，请以实时页面为准。完整对比与详情请查看 [TokenPlan](https://tokenplan.vip/)。

## 参与更新

欢迎提交 Issue / PR 修正以下信息：

- 价格变化
- 套餐名称变化
- 模型名称变化
- TokenPlan 详情页变化
- 套餐权益描述变化

如果你经常需要比较 AI Token Plan 或模型 API 价格，建议 Star 收藏。你也可以 Fork 本仓库，维护一份适合自己或团队的模型选型表。
